...
使用範例:
 ...
 9. 繪製多個層次
     python3 plot_wrf_var.py -i wrfout_d01_2006-06-09_00:00:00 -V z -L 850,700,500 -o z_multi.png
 11. 指定視圖範圍
...
def plot_wrf_variable(ifn, var_plot='slp', level_p=-9999, output=None, colormap='jet', output_dir='.',
   ...
   # cart_proj = get_proj_params(ncfile)
    ...
    if view_x is not None or view_y is not None:
        ...
        print(f"已裁剪資料範圍至: x={x_start}:{x_end}; y={y_start}:{y_end}")
   ...
   #ax = fig.add_subplot(1, 1, 1, projection=cart_proj)
    ...
    if gridlines:
        try:
            ...
            lon_contour = bm.contour(x_bm, y_bm, lons_np, levels=lon_levels,
                                    colors='gray', linewidths=0.4, alpha=0.8)
    ...
    if coastline_option == 0:
        ...
        try:
            ...
            ax.contour(x_bm, y_bm, landmask_np, levels=[0.9], colors='black', linewidths=2.0)
            ax.contour(x_bm, y_bm, landmask_np, levels=[0.9], colors='yellow', linewidths=0.4)
        except Exception as e:
            print(f"警告: 使用landmask繪製海岸線時出錯: {str(e)}")
    elif coastline_option == 2:
        # 使用basemap函數繪製
        try:
            bm.drawcoastlines(linewidth=2.0, color='black')
            bm.drawcoastlines(linewidth=0.4, color='yellow')
            bm.drawstates(linewidth=0.6, color='gray')
            bm.drawcountries(linewidth=0.6, color='gray')
        except Exception as e:
            print(f"警告: 使用basemap繪製海岸線時出錯: {str(e)}")
    else:
        print(f"警告: 無效的海岸線選項 {coastline_option}，應為0, 1或2")

    '''
    # 繪製海岸線 (使用landmask)
    if not no_coastline:
        try:
            ax.contour(landmask_np, levels=[0.9], colors='black', linewidths=2.0)
            ax.contour(landmask_np, levels=[0.9], colors='yellow', linewidths=0.4)
        except Exception as e:
            print(f"警告: 繪製海岸線時出錯: {str(e)}")

    # Add geographic outlines
    bm.drawcoastlines(linewidth=1.5, color='black')
    bm.drawstates(linewidth=0.6, color='gray')
    bm.drawcountries(linewidth=0.6, color='gray')
    '''
    ...
    # 繪製等值線填色
    try:
        cf = bm.contourf(x_bm, y_bm, VAR_2D, levels=levels, cmap=cmap, extend='both')
    except Exception as e:
        print(f"警告: 繪製shaded時出錯: {str(e)}")
        try:
            print("嘗試使用自動生成的色階範圍...")
            cf = bm.contourf(x_bm, y_bm, VAR_2D, cmap=cmap)
        except:
    ...
    if contour is not None and VAR2_2D is not None:
        try:
            ...
            ct = bm.contour(x_bm, y_bm, VAR2_2D, levels=ct_levels,
                         colors=contour_color, linewidths=contour_linewidth, alpha=1)
            if len(ct_levels) <= 30:     # 只有當等值線不太密集時才添加標籤
                ax.clabel(ct, inline=1, fontsize=9.5, fmt='%g')
    ...
    # 檔案名放在標題上方，字體較小
    ax.text(-0.1, 1.2, f"{ifn}", ha='left', va='top', transform=ax.transAxes, fontsize=6, alpha=1.0)
